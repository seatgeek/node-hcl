  ...
  try {
    return readFileSync(path, 'utf8');
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
          console.warn(`file not found at path ${path}, defaulting to empty string`);
          return '';
      } else {
...
export async function mergeWrite(a: string, b: string, outPath: string): Promise<void> {
...
export async function mergeFiles(aPath: string, bPath: string): Promise<string> {
...
export async function mergeFilesWrite(aPath: string, bPath: string, outPath: string): Promise<void> {
